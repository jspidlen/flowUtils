...
<<WriteGatingML2, echo=TRUE, eval=TRUE, results=verbatim>>=
flowEnv <- new.env()
...
The \texttt{spillRefId="SpillFromFCS"} indicates that the FCS data file shall
be searched for compenmsat 
...
<<WriteGatingML3, echo=TRUE, eval=TRUE, results=verbatim, split=TRUE>>=
