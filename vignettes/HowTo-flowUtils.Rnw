...
flowEnv <- new.env()
myRect <- rectangleGate(filterId="myRect", "FSC-H"=c(100, 700))
flowEnv[['myRect']] <- myRect
x <- tryCatch(write.gatingML(flowEnv), error = function(e) { e })
x$message
@
...
TODO: EXPLAIN this example (and why FlowRepository is showing 80.64\% while
R calculated 80.52\% in this gate).
...
TODO: Mention that there may be large differences the other way around due to
