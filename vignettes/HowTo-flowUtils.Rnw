...
In this example, we can also see how scaling transformations are mapped between
R and Gating-ML. Specifically, in Gating-ML 1.5 and in R, each scaling
...
\end{verbatim}
Next, we will use a \Rclass{quadGate} and demonstrate how scaling
...
\end{verbatim}
Next, we can modify the code so that compensated parameters are used as
arguments of the Logicle and ArcSinH transformations. In addition, we will use
these transformations directly rather than creating a transformation reference.
...
\end{verbatim}
So far, all our examples included a relativelly simple pipeline. Next, we
will try to demonstrate the use of a custom compensation along with a scaling 
transformation (log) applied to a ratio of two FCS parameters:
...
TODO If Gating-ML 2.0 is written, corresponding transformations will be
merged.\\

TODO Add example with unsupported transformation type\\

TODO These were all examples of meaningful data transformations and analysis
pipelines that can be saved in a Gating-ML 2.0 file. Below, we demonstrate...

<<WriteGatingMLIssue1, echo=TRUE, eval=TRUE, results=hide>>=
