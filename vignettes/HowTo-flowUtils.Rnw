...
To the best of our knowledge, R, FlowRepository
...
some of the data transformations to make Gating-ML based gating properly
interoperable.
...
(which has no effect on the calculation result).
...
of gates are ``meaningfull'' in terms of defining a ``useful'' population.
\\ 
...
We are using a
different approach to ensure that the gates' identifiers are
