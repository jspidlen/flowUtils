...
TODO Add example with unsupported transformation type; EXPLAIN:

<<WriteGatingMLUnsupportedCase1, echo=TRUE, eval=TRUE, results=hide>>=
flowEnv <- new.env()
tSS <- splitscale(parameters = "FL1-H", r = 1024, maxValue = 10000, 
    transitionChannel = 256, transformationId = "tSS")
myRect <- rectangleGate(filterId="myRect", list("logicle2"=c(100, 700)))
...
<gating:Gating-ML ... >
  ...
  <gating:RectangleGate gating:id="myRect">
    <gating:dimension gating:min="100" gating:max="700" 
      gating:transformation-ref="tSS" gating:compensation-ref="uncompensated">
      <data-type:fcs-dimension data-type:name="FL1-H"/>
    </gating:dimension>
  </gating:RectangleGate>
</gating:Gating-ML>
...
\begin{verbatim}
Warning message:
Class 'splitscale' is not supported in Gating-ML 2.0 output. Only Gating-ML 2.0 
compatible transformations are supported by Gating-ML 2.0 output. Transformation 
'tSS' is not among those and will be skipped. Therefore, any gate referencing 
this transformation will be referencing a non-existent transformation in the 
Gating-ML output.  
\end{verbatim}
}
...
You will need the \Rpackage{gatingMLData} package (version 2.2.0 or newer) and
the \Rpackage{RUnit} package in order to run the compliance tests.
