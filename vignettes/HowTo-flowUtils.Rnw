...
We did not show the output the \texttt{ls} commands above since these
...
@
We can see that the parameters are references to two transformations. We can
explore these further by entering
...
applied to ``PE-A'', which has been compensated according to the description in
the data file (\textit{e.g.}, the \texttt{\$SPILLOVER}, \texttt{SPILL} or other
keywords). Analogically, we can see that \texttt{Tr\_Arcsinh.FCS.APC-Cy7-A} is
an ArcSinH transformation applied to ``APC-Cy7-A'', which has also been
compensated according to the description in the data file.\\

\subsection{Scaling transformation shared in Gating-ML 2.0, not in R}
\label{sec:sharedTransforms}
In the previous example, we can also see how scaling transformations are
mapped between R and Gating-ML. Specifically, in Gating-ML 1.5 and in R, each scaling
transformation is bound to the parameter (\textit{e.g.}, FCS parameter) that it
is supposed to be applied to. We can see from the chunk of Gating-ML 1.5 code
below that transformations ``T1'' and ``T2'' are the same exept one is applied
to ``FL1-H'' while the other one to ``FL2-H''. In Gating-ML 1.5 and in R, these
transformations have to be defined separately, each of them having a unique
identifier assigned. 

\begin{verbatim}
<!-- This is a chunk of Gating-ML 1.5 code -->
...
\end{verbatim}
This design has been mainly driven by the fact that Gating-ML 1.5 and R allow
for an arbitrary combination of data transformations, although most of these
compound transformations are likely not very meaningful for the analysis of flow
cytometry data. On the other hand, Gating-ML 2.0 supports only
...
transformed, and therefore, Gating-ML 2.0 description is simpler and allows 
for the same transformation to be ``shared'' when applicable to multiple
FCS parameters. We can see the design difference in the chunk of
Gating-ML 2.0 code below:

\begin{verbatim}
<!-- This is a chunk of Gating-ML 2.0 code -->
<transforms:transformation transforms:id="T1">
...
is being reused for different FCS parameters (in one or more gates). New
...
transformations saved in the enviroment once this Gating-ML 2.0 chunk is read.
...
gate is created in the ``PE-A'' and ``PerCP-Cy5-5-A'' dimensions,
...
flowEnv2.0[['MySpill']]
@
Non-square spectrum matrices are extracted and saved the same way. For
...
``pipelines'' is expressible in Gating-ML 2.0 (see section
...
The example below demonstrates the inclusion of compensation in a Gating-ML
output. As before, we will programatically create objects that will be saved in
Gating-ML 2.0 output.
...
the measured and ``compensated'' values). The following piece of code
...
The previous code can be easily modified so that compensated
parameters are used as arguments of the Logicle and ArcSinH transformations. In
addition, we will use these transformations directly rather than creating a 
transformation reference.
Consequently, the \Rfunction{write.gatingML} function will create the
``trArcSinH1'' and ``trLogicle1'' transformations even without having to save
...
Code shown below demonstrates that this ``conversion'' is done automatically
when the \Rfunction{write.gatingML} is called:
...
So far, our examples included a relativelly simple gates. Next, we
...
Gating-ML 2.0 output. This behaviour can be demonstrated on the following
example:
<<WriteGatingML11, echo=TRUE, eval=TRUE, results=hide>>=
flowEnv <- new.env()
logicle1 <- logicletGml2(parameters = "FL1-H", T = 10000, M = 4.5, A = 0, W= 0.5, "logicle1")
logicle2 <- logicletGml2(parameters = "FL2-H", T = 10000, M = 4.5, A = 0, W= 0.5, "logicle2")
lin1 <- lintGml2(parameters = "FL1-H", T = 10000, A = 0, "lin1")
lin2 <- lintGml2(parameters = "FL2-H", T = 10000, A = 0, "lin2")
rectG <- rectangleGate(filterId="rectG", "logicle1"=c(0.2, 0.6), "lin2"=c(0.2, 0.6))
...
gates in the ``logicle2'' and ``lin1'' dimensions, respectivelly.
Due to the transformation merging, only the ``logicle1'' and ``lin1''
transformations are in the Gating-ML 2.0 output. The second dimension of
...
reference the ``logicle1'' transformation applied on ``FL2-H''.

\subsection{Example with unsupported pipelines}
As explained in section \ref{sec:gatingML2Pipelines}, not all
pipelines (workflows) expressible in R are expressible in Gating-ML 2.0.
...
environment that is being written to Gating-ML 2.0 output.
...
reference to it from the environment before being able to save it in Gating-ML
2.0.

\subsection{Example with unsupported gate type}
All widely used static gates are Gating-ML 2.0 compatible. 
Gates that are not compatible with Gating-ML 2.0 include $n$-dimension polytope
gates (\Rclass{polytopeGate} class) and decision tree gates
(\Rclass{expressionFilter}). These types of gates are supported by Gating-ML
1.5, but the support has been removed in Gating-ML 2.0 since these gates are
...
A similar error is produced if we try to include data driven gates in the
Gating-ML output as shown below: 
...
The flowUtils package includes RUnit-based compliance tests to verify its
compliance with the Gating-ML 1.5 and Gating-ML 2.0 specifications. 
You will need the \Rpackage{gatingMLData} package (version 2.2.0 or newer) 
...
Once you have the required packages installed, the 
\Rfunction{testGatingMLCompliance} function can be used to test Gating-ML
...
@
This code will execute 365 test functions and should take about 4 -- 6 minutes
...
also testing of various Quad gates in combination with compensation, ratio and
...
populations calculated by R (which is precise) and by other tools (which may be
approximate).
...
This can be demonstrated on the following example. We have a taken a randomly
...
that FlowRepository incorporates binning (with 256 bins) in gating.\\ 

TODO: Additional very minor differences can be observed due to different
``channel to scale'' transformations implemented in different software tools. In
R, we are using the FCS 3.1 \citep{paper:FCS3.1Paper} and Gating-ML 2.0
compatible formula:
...
% Mention that there may be large differences the other way around due to
third party software not implementing all the transformations etc.
