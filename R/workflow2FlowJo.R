...
## as 'a'.
## The Populations XML node contained within epresents a single gating node in 
...
## workspace. NULL values are ignored.                                 
## FIXME: This should be in an XML file and be parsed on package attach.
fjSettings <- function()
  list(
    Workspace=list(
      version="1.6",
      flowJoVersion="7.5b134",
      modDate=format(Sys.time(), "%a %b %d %H:%M:%S %Z %Y"),
      useFreqParent="1",
      drawRowBorders="1",
      drawColumnBorders="1",
      isTemplate="0",
    ...
    "gating:PolygonGate"=list(
      ...
      lineWeight="Hairline",
      isFJGate="1"),
    "gating:dimension"=list(
      "gating:min"="0",
      "gating:max"="0"),
    "gating:coordinate"=list(
      "data-type:value"="0"),
    "gating:vertex"=list(),
    "data-type:parameter"=list(
      "data-type:name"="anonymous"),
    TableEditor=list(
      title="anonymous",
      current="Untitled"),
    Table=list(
