...
addAsinhtGml2 <- function(gatingMLNode, x, flowEnv)
{
    ...
    attrs = c("transforms:id" = myID)
    if (("boundMin" %in% names(getSlots(class(myTrans)))) && is.finite(myTrans@boundMin)) attrs = append(attrs, c("transforms:boundMin" = myTrans@boundMin))
    if (("boundMax" %in% names(getSlots(class(myTrans)))) && is.finite(myTrans@boundMax)) attrs = append(attrs, c("transforms:boundMax" = myTrans@boundMax))
